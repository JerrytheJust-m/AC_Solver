"""
ppo_training_loop function implements the training loop logic of PPO, with multiple environments
with randomnessn depending on rela
"""
...
def ppo_training_loop(
    ...
    #each copy of the environment is equipped with a randomness, which lies in (0, 1).
    #TODO: move this randomness arguement into args. Here assume num-envs = 2
    randomness = [0.99, 0.01]
    ...
    for update in tqdm(
        ...
        for step in tqdm(
            ...
            with torch.no_grad():
                #here use the agent from research sibling method which has randomness parameters.
                action, logprob, _, value = agent.get_action_and_value(next_obs, randomness)  # shapes: n_envs, n_envs, n_envs, (n_envs, 1)
                    ...
                    if done[i]:
                        ...
                        if curr_states[i] not in ACMoves_hist:
                            ...
                            if new_path_length < prev_path_length:
                                ACMoves_hist[curr_states[i]] = infos[
                                    "actions"
                                ]
        ...
        if (
            ...
            # TODO: check how the newly added "unwrapped" command solved the error: "'TransformReward' object has no attribute 'max_reward'"
            rewards /= envs.envs[0].unwrapped.max_reward
            normalized_returns = np.array(returns_queue) / envs.envs[0].unwrapped.max_reward
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue = agent.get_action_and_value(
                    b_obs[mb_inds], randomness, b_actions.long()[mb_inds]
        ...
        if update > 0 and update % 5 == 0:  # save a checkpoint every 100 updates
            checkpoint = {
                ...
                # "supermoves": envs.envs[0].unwrapped.supermoves,  # dict of supermoves or None
            }
